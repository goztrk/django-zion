...
from zion.apps.js_urls.constants import (
    URL_ARG_RE,
    URL_KWARG_RE,
...
from zion.utils.strings import replace
...
def _parse_resolver(
    ...
    include_all = include_all or (namespace in settings.ZION_JS_URLS_ALLOWED)
    ...
    for url_pattern in resolver.url_patterns:
        if isinstance(url_pattern, URLResolver):
            ...
            url_name = (
                f"{namespace}:{url_pattern.name}" if namespace else url_pattern.name
            )
            if url_name in settings.ZION_JS_URLS_ALLOWED or include_all:
